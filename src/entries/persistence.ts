...
import type { Expense, ExpenseSplit } from '../types.ts'
...
export async function writeExpense(expense: Expense, splits: ExpenseSplit[], database: KakeiboDB = db): Promise<ExpenseUndo> {
  return database.transaction('rw', database.expenses, database.expenseSplits, async () => {
    ...
    return { before, beforeSplits, saved, savedSplits }
  })
}
